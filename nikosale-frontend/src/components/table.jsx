...
const Avatar = ({name}) => {
    ...
    return (
        <div className={`flex items-center justify-center rounded-full w-8 h-8 text-white text-xs font-semibold ${bg}`}>
            {initial}
        </div>
    );
};
...
export const ManagerTable = ({managers, loading, onDelete}) => {
    ...
    return (
        <div className="w-full mt-6">
            {/* Desktop Table */}
            <div className="hidden sm:block rounded-2xl border overflow-x-auto bg-card shadow-sm">
            ...
            <div className="sm:hidden space-y-4">
                {managers.map((manager) => (
                    <div
                        key={manager.id}
                        className="flex items-start justify-between gap-4 p-4 border rounded-xl bg-card shadow-sm"
                    >
                        <div className="flex gap-3 items-start">
                            ...
                            <div className="space-y-0.5">
                                ...
                                <span
                                    className="text-xs bg-muted px-2 py-0.5 rounded-full inline-block mt-1 text-muted-foreground">
                  {manager.role}
                </span>
                            </div>
                        </div>
